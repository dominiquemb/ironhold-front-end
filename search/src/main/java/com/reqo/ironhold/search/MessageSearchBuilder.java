...
public class MessageSearchBuilder {
    ...
    public MessageSearchBuilder withSort(IndexFieldEnum sortField, SortOrder sortOrder) {
    ...
    public SearchRequestBuilder build() {
        if (id == null) {
            ...
            if (fromFacetValues.size() > 0 || fromDomainFacetValues.size() > 0 ||
                ...
                if (fromFacetValues.size() > 0) {
                    andFilter.add(FilterBuilders.inFilter(IndexFieldEnum.FROM_NAME.getValue(),
                            fromFacetValues.toArray(new String[fromFacetValues.size()])));
                }
                if (fromDomainFacetValues.size() > 0) {
                    andFilter.add(FilterBuilders.inFilter(IndexFieldEnum.FROM_ADDRESS.getValue(),
                            fromDomainFacetValues.toArray(new String[fromDomainFacetValues.size()])));
                }

                if (toFacetValues.size() > 0) {
                    andFilter.add(FilterBuilders.inFilter(IndexFieldEnum.TO_NAME.getValue(),
                            toFacetValues.toArray(new String[toFacetValues.size()])));
                }
                ...
                if (dateFacetValues.size() > 0) {
                    ...
                    for (Long yearFacetValue : dateFacetValues) {
                        ...
                        orFilter.add(FilterBuilders.numericRangeFilter(IndexFieldEnum.DATE.getValue()).from(from
                                .toDate().getTime()).to(to.toDate().getTime()));
                    }


                    andFilter.add(orFilter);

                    //..inFilter(IndexFieldEnum.DATE.getValue(), dateFacetValues.toArray(new Long[dateFacetValues
                    // .size()])));
                }

                FilteredQueryBuilder fqb = QueryBuilders.filteredQuery(qb, andFilter);
            ...
            FilteredQueryBuilder fqb = QueryBuilders.filteredQuery(QueryBuilders.queryString(criteria),
        ...
        builder.addHighlightedField(IndexFieldEnum.ATTACHMENT.getValue(), 100, 1);
        ...
        if (dateFacet) {
            builder.addFacet(FacetBuilders.dateHistogramFacet(FACET_DATE).interval("year").field(IndexFieldEnum.DATE
                    .getValue()));
        }

        if (fromFacet) {
            builder.addFacet(FacetBuilders.termsFacet(FACET_FROM_NAME).field(IndexFieldEnum.FROM_NAME.getValue()));
        }

        if (fromDomainFacet) {
            builder.addFacet(FacetBuilders.termsFacet(FACET_FROM_DOMAIN).field(IndexFieldEnum.FROM_ADDRESS.getValue()));
        }

        if (toFacet) {
            builder.addFacet(FacetBuilders.termsFacet(FACET_TO_NAME).field(IndexFieldEnum.TO_NAME.getValue()));
        }

        if (toDomainFacet) {
            builder.addFacet(FacetBuilders.termsFacet(FACET_TO_DOMAIN).field(IndexFieldEnum.TO_ADDRESS.getValue()));
        }
