...
public abstract class FileWatcher {
	...
	protected void processFiles(List<String> dataFileNames,
	...
	public FileWatcher(String inputDirName, String outputDirName)
		...
		logger.info("Watching " + inputDirName + " directory");
		...
		while (true) {
			...
			for (WatchEvent<?> event : watckKey.pollEvents()) {
				...
				if (fileName.endsWith(".md5")) {
					...
					List<String> dataFiles = processChecksumFile(inputDirName,
							fileName);
					if (dataFiles != null) {
						processFiles(dataFiles, fileName);
						for (String dataFile : dataFiles) {
							new File(inputDirName, dataFile).renameTo(new File(
									outputDirName, dataFile));
						}
					}
				}
	...
	private List<String> processChecksumFile(String inputDirName,
		...
		for (String line : lines) {
			...
			if (lineChunks.length != 2) {
				throw new Exception("Error processing " + fileName + ": '"
			...
			File dataFile = new File(inputDirName + File.separator
					+ dataFileName);
	...
	private boolean verifyChecksum(String checkSum, File dataFile)
			throws Exception {
